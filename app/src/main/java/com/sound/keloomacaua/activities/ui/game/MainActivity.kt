...
class MainActivity : AppCompatActivity() {
    ...
    var bottomCardsAdaptor: MyCardDisplayAdapter? = null
    var opponentCardsAdapter: OpponentCardsAdapter? = null
    var cardsInHand: RecyclerView? = null
    var opponentCards: RecyclerView? = null
    var imgTopCard: ImageView? = null
    var btnTakeCards: TextView? = null
    var btnDone: Button? = null
    var txtOpponentCardsCount: TextView? = null
    var btnClubs: View? = null
    var btnDiamonds: View? = null
    var btnHearts: View? = null
    var btnSpades: View? = null
    var imgSuiteOverride: ImageView? = null
    ...
    private fun updateStateFromGame(game: Game) {
        ...
        if (game.activeSkipTurns > 0 && game.playerToSkipTurn == localPlayerIndex) {
            btnDone!!.text = if (game.activeSkipTurns > 1) getString(R.string.skip_n_turns, game.activeSkipTurns) else getString(R.string.skip_1_turn)
        } else {
        ...
        if (opponentCardsCount >= 0) {
            txtOpponentCardsCount!!.text = getString(R.string.opponent_cards_count, opponentName, opponentCardsCount)
