...
class LobbyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (user == null) {
            ...
            userId = user.uid
            username = user.displayName ?: getString(R.string.waiting_for_player)
        }
        recyclerView = findViewById(R.id.gameList)
        lobbyAdapter = LobbyAdapter(userId, username)
        recyclerView.adapter = lobbyAdapter
        dataRef.child(Constants.DB_COLLECTION_GAMES)
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val gameList = mutableListOf<Game>()
                    for (gameSnapshot in snapshot.children) {
                        val game = gameSnapshot.getValue(Game::class.java) as Game
